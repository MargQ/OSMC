...
def Spectrum_NS_second(mass_bit_np_gold_crc,mass_np_samples,signal,signal_noise_orig):   
    ...
    plt.figure(10)
    plt.subplot(1,2,1)
    plt.title("Спектр сигналал с разными значениями отсчетов на бит RX")
    print(len(w))
...
print("Gold = ", Gold())
mass_bit_list_gold_crc = Gold() + mass_bit_list + CRC(mass_bit_list)
...
Spectrum_NS_second(mass_bit_np_gold_crc,mass_np_samples,signal,signal_noise_orig)
